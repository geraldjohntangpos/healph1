...
<?php
	...
	<?php
						...
						foreach($retrieve2 as $row2) {
							...
							<tr style="width: 100%; min-height: 90px; background-color: #fff;">
								...
								<td style="padding: 15px; width: 10%; background-color: green;">
									<a href="getproductcomments.php?productid=<?php echo $labelid; ?>&q=confirmnotif&clientid=<?php echo $clientid; ?>">
										<center>
		...
		function confirmFunc($reserveid, $healerid, $labelid, $clientid, $conn) {
			...
			if($update) {
				$sql2 = "UPDATE notification SET STATUS = 'CONFIRMED' WHERE NOTIF_OWNER = '$healerid' AND TYPE_ID = '$reserveid' AND TYPE = 'reservation' AND STATUS = 'ACTIVE'";
